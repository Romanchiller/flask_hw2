...
class AdvView(views.MethodView):
    ...
    def post(self):
        ...
        print(x)
        y = hash_password(password)
        print(y)
        print(check_password(password, x))
        print(check_password(password, y))



        user = request.session.query(User).filter(User.name == user_name).first()
        print(user.name, user.password)
        print(check_password(user.password, password))
        if user is None or check_password(user.password, password) is False:
            return HttpError(401, 'Not authorized user')
